<script>
  ...
  async function handleUpload(ev) {
    ...
    try {
      ...
      const { error } = await supabase.storage.from('articles').upload(filePath, file, { upsert: false });
      if (error && !String(error.message || '').includes('The resource already exists')) {
        console.error(error);
  ...
  async function loadArticle(s) {
    try {
      ...
      slug = data.slug || '';
      body = data.body || '';
      meta = data.data || { excerpt: '', heroImage: '', heroAlt: '', author: { name: '', role: '' }, tag: '' };
      selectedSlug = slug;
      message = '';
    } catch (e) {
      console.error(e);
...
<div class="max-w-6xl mx-auto">
  <div class="flex items-center justify-between mb-4">
    ...
    <button class="px-3 py-1.5 text-sm text-white rounded bg-primary-600 hover:bg-primary-700" on:click={newArticle}>Nouvel article</button>
  </div>
  <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
    <!-- Editor -->
    <div class="md:col-span-2 p-3 border border-gray-700 rounded-lg bg-gray-800/40">
      <div class="flex items-center justify-between mb-2">
        <div class="text-sm">Contenu</div>
        <label class="flex items-center gap-2 text-xs">
          <span class="opacity-80">Ajouter un fichier</span>
          <input type="file" class="text-xs file:mr-4 file:py-1 file:px-2 file:rounded file:border-0 file:text-xs file:bg-gray-700 file:text-white hover:file:bg-gray-600" on:change={handleUpload} />
        </label>
      </div>
  <CartaEditor {carta} bind:value={body} />
    ...
    <div class="flex flex-col gap-4 p-3 border border-gray-700 rounded-lg bg-gray-800/40">
      <div>
        ...
        <input id="search" placeholder="Rechercher..." class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded" bind:value={search} />
      </div>
      <div class="max-h-64 overflow-auto border border-gray-700 rounded">
        {#if loadingList}
        ...
        {:else}
          <ul class="divide-y divide-gray-700">
            {#each articles.filter(a => a.title?.toLowerCase().includes(search.toLowerCase())) as a}
              <li>
                <button type="button" class={`w-full text-left p-2 hover:bg-gray-700/40 cursor-pointer rounded ${a.slug===selectedSlug ? 'bg-gray-700/30' : ''}`} on:click={() => loadArticle(a.slug)}>
                  <div class="text-sm font-medium">{a.title}</div>
                  <div class="text-xs text-gray-400">{a.slug}</div>
                  {#if a.date}
                    <div class="text-[10px] text-gray-500">{new Date(a.date).toLocaleDateString('fr-FR')}</div>
                  {/if}
                </button>
              </li>
            {/each}
      ...
      <div>
        ...
        <input id="title" class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded" bind:value={title} on:input={() => (slug = toSlug(title))} />
      </div>
      <div>
        <label class="block mb-1 text-sm" for="slug">Slug</label>
        <input id="slug" class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded" bind:value={slug} />
      </div>
      <div>
        <label class="block mb-1 text-sm" for="excerpt">Extrait</label>
        <textarea id="excerpt" rows="3" class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded" bind:value={meta.excerpt} />
      </div>
      <div>
        <label class="block mb-1 text-sm" for="tags">Tags (séparés par espaces, virgules ou #)</label>
        <input id="tags" class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded" bind:value={meta.tag} />
      </div>
      <div>
        <label class="block mb-1 text-sm" for="heroImage">Image de couverture (URL)</label>
        <input id="heroImage" class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded" bind:value={meta.heroImage} />
        <div class="mt-2 flex items-center gap-2">
          <input type="file" accept="image/*" on:change={handleCoverUpload} class="text-xs file:mr-4 file:py-1 file:px-2 file:rounded file:border-0 file:text-xs file:bg-gray-700 file:text-white hover:file:bg-gray-600" />
          {#if meta.heroImage}
            <img src={meta.heroImage} alt="couverture" class="h-10 w-10 object-cover rounded border border-gray-700" />
          {/if}
        </div>
      </div>
      <div>
        <label class="block mb-1 text-sm" for="heroAlt">Texte alternatif</label>
        <input id="heroAlt" class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded" bind:value={meta.heroAlt} />
      </div>
      <div class="grid grid-cols-2 gap-2">
        <div>
          <label class="block mb-1 text-sm" for="authorName">Auteur</label>
          <input id="authorName" class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded" bind:value={meta.author.name} />
        </div>
        <div>
          <label class="block mb-1 text-sm" for="authorRole">Rôle</label>
          <input id="authorRole" class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded" bind:value={meta.author.role} />
        </div>
      </div>

      <button class="px-4 py-2 text-white rounded bg-primary-600 hover:bg-primary-700 disabled:opacity-50" on:click|preventDefault={save} disabled={saving}>
